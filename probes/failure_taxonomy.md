...
|---|---|---|---|
| `model_intent_misclassification` | Reply is labeled with the wrong intent | LLM | `failure_attribution.failure_stage=model_intent` |
| `low_confidence_intent` | Intent confidence below threshold | LLM / handoff policy | `model_stage.confidence < threshold` |
| `crm_lookup_not_found` | HubSpot lookup returns no contact | HubSpot / CRM state | `execution_steps[].status=not_found` |
| `crm_update_failed` | HubSpot update returns false/error | HubSpot runtime | `component=hubspot`, `step=hubspot_update_contact` |
| `calendar_link_failure` | Cal.com link generation fails | Cal.com service | `component=cal`, `step=cal_link_generation` |
| `email_delivery_failure` | Resend returns error/timeout | Resend service | `component=resend`, `step=resend_email` |
...
| `business_outcome_failed` | Tools ran but booking link/email not delivered | End-to-end pipeline | `business_outcome.email_sent=false` or `booking_link_generated=false` |
